...
| D20 | Main window direction | Fixed tray popup with Focus/Dashboard toggle - resizable dashboard app - separate compact/full windows | In progress: **resizable responsive dashboard** | The glance widget now covers the quick-focus job. The main window is becoming a normal resizable dashboard app: no pin control, no main-window always-on-top behavior, visible in the taskbar, and responsive compact/dashboard layouts based on window size. |
