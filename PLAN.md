...
| D12 | Provider management surface | Global settings sheet · provider detail pages · per-card popovers | ✅ **Updated: provider detail pages** | The resizable dashboard makes providers first-class sections. The sidebar has an All view plus provider pages; provider-specific connect/disconnect, DeepSeek key management, and OpenCode Go linking live on each provider page. Settings stays app-level only: theme, glance, and refresh cadence. |
...
| D15 | Tray placement | Always tray-attached · floating/popped-out mode · separate pinned window | Superseded by D19/D20 | The earlier header pin / popped-out mode was removed when the glance window took over quick visibility and the main window became a normal resizable dashboard app. Tray icon click now opens the dashboard; the separate glance window remains draggable and always-on-top. |
...
| D19 | Glance surface | Tray tooltip/badge · draggable glance widget · taskbar overlay/native shell extension | ✅ **Decided: draggable always-on-top glance widget** | Windows taskbar overlay behavior is too brittle for the long run. Keep the glance surface as a small draggable always-on-top window with remembered position. It shows starred providers first, using `current | weekly` for limit providers and a single balance value for balance-only providers such as DeepSeek. The tray icon still opens the full dashboard. |

| D20 | Main window direction | Fixed tray popup with Focus/Dashboard toggle - resizable dashboard app - separate compact/full windows | In progress: **resizable responsive dashboard** | The glance widget now covers the quick-focus job. The main window is becoming a normal resizable dashboard app: no pin control, no main-window always-on-top behavior, visible in the taskbar, and responsive compact/dashboard layouts based on window size. The dashboard model is now All providers plus provider-specific detail pages with future analytics placeholders. |
...
- [x] Add first dashboard navigation layer: All providers view, provider-specific detail pages, provider setup moved out of Settings, disconnected providers hidden/restorable from Add Provider, starred providers sorted to the top, and starred providers feeding the glance window with existing defaults as fallback.
